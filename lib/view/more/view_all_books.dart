...
class ViewAllBooks extends StatefulWidget {
  ...
  const ViewAllBooks({super.key, required this.allBooks});
...
class _ViewAllBooksState extends State<ViewAllBooks> {
  ...
  final List<String> categories = [
    "All", "Fiction", "Classic", "Romance", "Mystery", "Fantasy", "History", "Comic", "Crime"
  ];
  ...
  void initState() {
    super.initState();
    // Calculate price ranges dynamically based on the books' prices
  ...
  Future<void> _fetchAllBooks() async {
    // This method can be implemented if you want to fetch all books from Firestore again
    setState(() {
      isLoading = false;
    });
  }

  // Function to generate price ranges dynamically
  ...
  List<Map<String, dynamic>> _filterBooks() {
    return widget.allBooks.where((book) {
      bool matchesCategory = selectedCategory == 'All' || book['category'] == selectedCategory;
      bool matchesPrice = book['price'] >= selectedPriceRange.start && book['price'] <= selectedPriceRange.end;
  ...
  Widget build(BuildContext context) {
    return bgWidget(
      child: Scaffold(
        key: _scaffoldKey, // Global key to control the Scaffold's state
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                _scaffoldKey.currentState?.openDrawer(); // Open the drawer for filter options
              },
            ),
          ],
        ),
        ...
        body: isLoading
            ...
            : _filterBooks().isEmpty
                ? const Center(child: Text('No books available for selected filters'))
                : GridView.builder(
  ...
  // Filter Drawer with category and price range options
              ...
              children: priceRanges.map((priceRange) {
                return RadioListTile<RangeValues>(
                  ...
                  onChanged: (value) {
                    ...
                    Navigator.pop(context); // Close the drawer after selecting a price range
                  },
